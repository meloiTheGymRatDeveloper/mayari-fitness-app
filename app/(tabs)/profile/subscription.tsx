...
export default function SubscriptionScreen() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<'beta' | 'monthly' | 'yearly'>(
    IS_IOS ? 'monthly' : 'beta',
  );
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          {!isActive && (
            <View style={styles.offerCard}>
                ...
                {IS_IOS ? (
                  iapStatus === 'ready' ? (
                    <Text style={styles.offerPrice}>
                      {getIosPrice(selectedPlan === 'yearly' ? 'yearly' : 'monthly') ?? '—'}
                    </Text>
                  ) : iapStatus === 'initializing' ? (
                    <View style={styles.priceLoadingRow}>
                      <ActivityIndicator color={colors.brand.primary} />
                      <Text style={styles.priceLoadingText}>Loading App Store pricing…</Text>
                    </View>
                  ) : (
              ...
              {!IS_IOS && selectedPlan === 'monthly' && (consistencyDiscount > 0 || referralDiscount > 0) && (
              ...
              {(() => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={IS_IOS ? handleIOSSubscribe : handleAndroidSubscribe}
                    disabled={disabled}
                  >
          ...
          {isActive && (
            <TouchableOpacity
              style={[styles.cancelBtn, !IS_IOS && cancelSub.isPending && styles.disabled]}
...
const styles = StyleSheet.create({
  ...
  priceLoadingRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, paddingVertical: spacing.xs },
  priceLoadingText: { color: colors.text.secondary, fontSize: typography.sm, fontFamily: fonts.regular },
  priceUnavailable: { color: colors.warning, fontSize: typography.sm, fontFamily: fonts.semibold, paddingVertical: spacing.xs },
  iapErrorHint: { color: colors.text.secondary, fontSize: typography.xs, fontFamily: fonts.regular, marginTop: spacing.xs },
  betaBanner: {
  ...
  restoreBtnText: {
    color: colors.brand.primary, fontSize: typography.sm, fontFamily: fonts.medium,
  },
  ...
  referralDiscountActive: { color: colors.success, fontSize: typography.sm, fontFamily: fonts.semibold },
  ...
  paymentFailedCard: {
    ...
    borderWidth: 1.5, borderColor: colors.warning, padding: spacing.md,
  },
  paymentFailedTitle: { color: colors.warning, fontSize: typography.base, fontFamily: fonts.bold, marginBottom: spacing.xs },
  paymentFailedBody: {
    color: colors.text.secondary, fontSize: typography.sm, fontFamily: fonts.regular,
  ...
  paymentFailedBtnText: { color: '#000', fontSize: typography.sm, fontFamily: fonts.bold },
});
