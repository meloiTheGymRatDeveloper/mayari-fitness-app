...
const STRAVA_CLIENT_ID = "254812"; // Fill in from Strava API application settings
...
export function useSyncFromStrava() {
  ...
  return useMutation({
    mutationFn: async (workoutType: "running" | "cycling") => {
      ...
      return data as {
        session_id: string;
        cardio_metrics_id: string;
        advanced_week: boolean;
      };
    },
    onSuccess: (_, workoutType) => {
      queryClient.invalidateQueries({
        queryKey: ["cardio_sessions", workoutType, userId],
      });
      queryClient.invalidateQueries({
        queryKey: ["cardio_enrollment", workoutType, userId],
      });
      queryClient.invalidateQueries({
        queryKey: ["workout_sessions", "recent", userId],
      });
    },
  });
}
